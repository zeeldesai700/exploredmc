      ...
      <table class="table table-bordered table-sm small align-middle text-center">
        ...
        <tbody class="hotelBody">
          <tr class="hotel-row">
             <td>
  <input type="date"
    name="hotel[1][stay_date][]"
    class="form-control form-control-sm hotel-date"
    readonly>
</td>
...
function recalculateHotelDates(optionBox) {
  ...
  let currentDate = new Date(travelDateVal);

  optionBox.find('.hotelBody tr').each(function () {

    const row = $(this);
    const nights = parseInt(row.find('.stay').val()) || 1;
    ...
    row.find('.hotel-date').val(`${yyyy}-${mm}-${dd}`);

    // move date forward by nights
