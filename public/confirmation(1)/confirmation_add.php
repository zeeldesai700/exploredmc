...
$resTravel = $conn->query("
    ...
    LEFT JOIN cities c ON qt.city_id = c.id
    LEFT JOIN pickup_points pp ON qt.pickup_point_id = pp.id
    LEFT JOIN meals m ON qt.meal_id = m.id
    LEFT JOIN sightseeings ss ON qt.sightseeing_id = ss.id
    WHERE qt.quotation_id = $quotation_id
    ORDER BY qt.day_date ASC, qt.day_no ASC, qt.id ASC
");
...
<td>
  <input type="number"
         ...
         placeholder="Amount">
          ...
          <input type="hidden" name="hotels[<?= $optionNo ?>][<?= $i ?>][stay_nights]"
                 value="<?= (int)$r['stay_nights'] ?>">

          <input type="hidden" name="hotels[<?= $optionNo ?>][<?= $i ?>][rooms]"
                 value="<?= (int)$r['rooms'] ?>">
        </tr>
        <?php endforeach; ?>
      </tbody>
...
<div class="table-responsive">
  ...
  <input type="text"
        ...
        <td>
          ...
          <input type="hidden"
                 name="travel[<?= $i ?>][guide]"
                 value="<?= htmlspecialchars($t['guide'] ?? '') ?>">
        </td>
