...
$(document).on('keydown', '.stay', function (e) {
  e.preventDefault();
});
...
function autoSelectFirstActivityForRow(r) {
  ...
  $.post("../fetch/fetch_activities.php", { sightseeing_id: sight_id }, function (list) {
    ...
    console.log("✅ Auto activity applied correctly:", selectedData, total);
...
<div class="border rounded p-2 mb-2 car-item" data-id="${car.id}">
  <div class="d-flex justify-content-between align-items-center">
    ...
    <div class="d-flex align-items-center gap-1">
      <button type="button" class="btn btn-outline-secondary btn-sm removeSameCar">−</button>
<button type="button" class="btn btn-outline-primary btn-sm addSameCar">+</button>
...
$("#saveCarSelection").on("click", function () {
  ...
  $("#carList .car-item").each(function () {
    ...
    for (let i = 0; i < qty; i++) {
      selected.push({ id, mode, price: rate });
      total += rate;
    }
  });
