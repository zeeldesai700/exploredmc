...
/* Header accent */
#hotelRoomPopup .modal-header,
#hotelRoomPopup .border-bottom {
...
/* Room rows */
#hotelRoomPopup tbody tr {
  background: #ffffff;
  border-radius: 10px;
  transition: all 0.2s ease;
}

#hotelRoomPopup tbody tr:hover {
...
/* LOWEST highlight */
#hotelRoomPopup .lowest-room {
  background: linear-gradient(90deg, #e8fff1, #ffffff);
...
/* Select button */
#hotelRoomPopup .selectHotelRoom {
  padding: 6px 18px;
  font-size: 13px;
  border-radius: 999px;
...
#hotelRoomPopup .selectHotelRoom:hover {
  background: linear-gradient(180deg,#22c55e,#16a34a);
  box-shadow: 0 6px 18px rgba(34,197,94,0.45);
}

/* Cancel button */
#hotelRoomPopup .btn-secondary {
  border-radius: 999px;
  padding: 6px 18px;
}
...
/* Scrollbar (Chrome) */
#hotelRoomList::-webkit-scrollbar {
  width: 6px;
}
#hotelRoomList::-webkit-scrollbar-thumb {
  background: #cbd5e1;
  border-radius: 6px;
}

/* 🔥 Selected hotel room highlight */
#hotelRoomList tr.active-room {
  background-color: #e6fff1 !important;
}

#hotelRoomList tr.active-room td {
  background-color: #e6fff1 !important;
}

/* Optional: strong border on selected row */
#hotelRoomList tr.active-room {
      ...
      <table class="table table-bordered table-sm small align-middle text-center">
        <thead class="table-light">
          <tr>
            ...
            <th style="width:7%">Hotel Category</th>
            <th style="width:52%">Hotel & Room Category</th>
        ...
        <tbody class="hotelBody">
          <tr class="hotel-row">
            ...
            <td>
              <input type="date"
                class="form-control form-control-sm hotel-from-date"
                name="hotel[1][from_date][]">

              <input type="date"
                class="form-control form-control-sm mt-1 hotel-to-date"
                name="hotel[1][to_date][]">
            </td>
            ...
            <!-- HOTEL + ROOM (🔥 ALL HIDDEN INPUTS INSIDE SAME TD) -->
            <td>
              <button type="button"
              ...
              <input type="hidden"
                name="hotel[1][base_price][]"
                class="base_price">

              <input type="hidden"
                name="hotel[1][extra_adult_price][]"
                class="extra_adult_price">

              <input type="hidden"
                name="hotel[1][child_price][]"
                class="child_price">

              <input type="hidden"
                name="hotel[1][nobed_price][]"
                class="nobed_price">
            </td>
...
<div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">
  <div>
    <h5 class="mb-0 fw-bold">Select Hotel & Room</h5>
    <small class="text-muted">
      Best available rooms for selected dates
    </small>
  </div>
...
<div id="hotelRoomList" class="px-1">
...
function applyAutoActivity(row, activity) {
 ...
 <script src="../assets/js/quotation.js?v=3.8"></script>
    <!-- Bootstrap JS bundle (includes Popper) -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>

<script>
/* =====================================================
   AUTO SELECT FIRST ACTIVITY – FINAL WORKING VERSION
===================================================== */

let autoActivityRow = null;

/* 1️⃣ Sightseeing Change */
$(document).on("change", ".sightseeing", function () {
  ...
  autoActivityRow = row;
  window.activeTravelRow = row; // keep existing logic

  /* CLEAR OLD VALUES */
  row.find('.activity-values').val('');
  row.find('.activity-data').val('');
  row.find('.activity-price').val('0');

  // DO NOT rebuild activityList here
  // quotation.js will fetch & render activities automatically
});


/* 2️⃣ After Activity Modal Fully Opens */
$('#activityPopup').on('shown.bs.modal', function () {

  if (!autoActivityRow) return;

  /* 🔒 DO NOTHING IF ACTIVITY ALREADY SET */
  const existing = autoActivityRow.find('.activity-data').val();
  if (existing && existing !== '') return;

  const waitRender = setInterval(function () {

    const boxes = $('#activityList input[type="checkbox"]');
    ...
    if (boxes.length > 0) {
      ...
      const id = first.val();
      const adult = Number(first.data('adult') || 0);
      const child = Number(first.data('child') || 0);
      ...
      const total =
        (adult * adultsTotal) +
        (child * (children + noBed));

      /* 🔥 CORRECT FORMAT */
      autoActivityRow.find('.activity-values').val(id);
      autoActivityRow.find('.activity-data').val(JSON.stringify(selectedData));
      autoActivityRow.find('.activity-price').val(total.toFixed(2));

      recalcAllRowCosts();

      console.log('✅ Auto activity applied safely');
    }
...
$(function () {
  ...
  $('#saveQuotation').off('click').on('click', function (e) {
    ...
    try {
      ...
      payload.travels = collectTravels();

      console.log('TRAVELS =>', payload.travels);

      /* =================================================
         ✅ FINAL BUSINESS RULE (FIXED)
      ================================================= */

      const headerCarId = $('#defaultCarId').val();

      // check if travel plan ACTUALLY has data
      const hasTravelPlan = payload.travels.some(t =>
        t.city_id || t.sightseeing_id || (t.cars && t.cars.length)
      );

      // 🚫 BLOCK only when travel exists but header car missing
      if (hasTravelPlan && !headerCarId) {
        alert('Please select Transport (Car) in Header because Travel Plan exists');
        $('#defaultCarId').focus();
        return;
      }

      // ✅ ALLOW save for hotel-only quotation
      // (NO travel plan, NO transport → OK)
...
<script>
/* =====================================================
   HOTEL DATE RANGE + NIGHT CALCULATION (UPDATED)
===================================================== */
function recalculateHotelDates(optionBox) {

  const travelDateVal = $('#travel_date').val();
  if (!travelDateVal) return;

  // start from travel date
  let currentDate = new Date(travelDateVal + 'T00:00:00');

  optionBox.find('.hotelBody tr').each(function () {

    const row = $(this);

    let nights = parseInt(row.find('.stay').val(), 10);
    ...
    row.find('.hotel-date').val(fromDate);

    /* ================= TO DATE ================= */
    const checkout = new Date(currentDate);
    checkout.setDate(checkout.getDate() + nights);

    const ty = checkout.getFullYear();
    const tm = String(checkout.getMonth() + 1).padStart(2, '0');
    const td = String(checkout.getDate()).padStart(2, '0');

    // OPTIONAL: if you add a to-date input later
    row.find('.hotel-to-date').val(`${ty}-${tm}-${td}`);
...
function applyHotelDateLimits() {

  const travelDate    = $('#travel_date').val();
  const departureDate = $('#departure_date').val();

  if (!travelDate || !departureDate) return;

  $('.hotel-row').each(function () {

    const row = $(this);
    const fromInput = row.find('.hotel-from-date');
    const toInput   = row.find('.hotel-to-date');

    // FROM date: only travel → departure
    fromInput
      .attr('min', travelDate)
      .attr('max', departureDate);

    // TO date: min = from + 1, max = departure
    if (fromInput.val()) {
      const d = new Date(fromInput.val() + 'T00:00:00');
...
/* 🔁 Recalculate when nights change */
$(document).on('input', '.stay', function () {
  const optionBox = $(this).closest('.hotel-option');
  recalculateHotelDates(optionBox);
});

/* 🔁 Recalculate + Restrict hotel dates when travel date changes */
$('#travel_date').on('change', function () {

  $('.hotel-option').each(function () {
    recalculateHotelDates($(this));
  });

  // 🔒 IMPORTANT: limit hotel calendars
  applyHotelDateLimits();
});

/* 🔁 When hotel FROM date changes */
$(document).on('change', '.hotel-from-date', function () {
  applyHotelDateLimits();
});


/* 🔁 Recalculate when new hotel row added */
$(document).on('click', '.addHotelRow', function () {
  const optionBox = $(this).closest('.hotel-option');
  setTimeout(() => recalculateHotelDates(optionBox), 50);
});
...
<script>
/* ===============================
   AUTO CALCULATE HOTEL NIGHTS
================================ */

function calculateHotelNights(row) {

  const fromVal = row.find('.hotel-from-date').val();
  const toVal   = row.find('.hotel-to-date').val();
  ...
  // ✅ ONLY SET NIGHTS
  row.find('.stay').val(diffDays);
}

/* 🔁 When FROM or TO date changes */
$(document).on('change', '.hotel-from-date, .hotel-to-date', function () {
  const row = $(this).closest('.hotel-row');
  calculateHotelNights(row);
});

/* 🔁 When new hotel row added */
$(document).on('click', '.addHotelRow', function () {
  setTimeout(() => {
    const optionBox = $(this).closest('.hotel-option');
    optionBox.find('.hotel-row').each(function () {
      calculateHotelNights($(this));
    });
  }, 100);
});
