...
function buildQuotationHtml(
    mysqli $conn,
    array $q,
    mysqli_result $hotels,
    mysqli_result $travels,
    string $visaText,
    string $tipText
): string {
...
<tr>
  ...
  <td style="text-align:center;">'.$q['adults'].'</td>
  <td style="text-align:center;">'.$q['children'].'</td>
  <td style="text-align:center;">'.$q['no_bed_child'].'</td>
</tr>
</table>
...
while ($h = $hotels->fetch_assoc()) {
    ...
    $to   = date('d M Y', strtotime("+{$h['stay_nights']} days", $startDate));
  ...
  <span style="font-size:10px;color:#555;">
    '.$h['hotel_category'].'
      ...
      <td class="center">
        '.$h['city'].'<br>
        <span style="font-size:10px;">'.$h['stay_nights'].' Night</span>
      </td>
      ...
      <td class="center">DBL : '.$q['rooms'].'</td>
...
'.showCostRow('Per Adult Cost', $q['grand_per_adult']).'
'.showCostRow('Per Adult Extra Bed Cost', $q['grand_per_extra_adult']).'
'.showCostRow('Per Child With Bed', $q['grand_per_child']).'
'.showCostRow('Per Child Without Bed', $q['grand_per_child_no_bed']).'
...
<tr>
  <td style="padding-top:6px;font-weight:bold;">
    TOTAL PACKAGE COST
  </td>
  <td style="padding-top:6px;font-weight:bold;text-align:center;">
    USD '.number_format($q['grand_total'],0).'/- 
  </td>
</tr>
...
while ($t = $travels->fetch_assoc()) {
    ...
    // Fetch multiple activities (from second table)
    $activityNames = get_travel_activities($conn, (int)$t['id']);

    if ($activityNames === '—' && empty($t['sightseeing_name'])) {
        continue;
    }

    $vehicle = $t['car_name']
        ? $t['car_name'].' ('.$t['seater'].' Seater)'
        : '—';
...
mysqli_data_seek($travels,0); while($t = $travels->fetch_assoc()){ if(!$t['car_id']) continue; $html .= '

    <tr>
      <!-- ACTIVITY -->
      <td>'.$t['sightseeing_name'].' Transfers</td>

      <!-- TYPE (ONLY PVT) -->
      <td class="center">PVT</td>

      <!-- VEHICLE -->
      <td>'.$t['car_name'].' ('.$t['seater'].' Seater)</td>
...
<b>Day '.$t['day_no'].' ('.date('d-m-Y',strtotime($t['day_date'])).')</b><br>
...
<ul>
  ...
  <li>All entrance fee, meals as mentioned in program (B – Breakfast, L – Lunch, D – Dinner).</li>
  <li>Mineral water: 2 bottles per person per day.</li>
</ul>

<div class="section-title">EXCLUSION</div>
<ul>
  <li>PCR Covid test or Rapid test before arrival. Guide available where mentioned, extra days guide charge extra.</li>
  <li>International airfare in & out Vietnam & internal domestic airfare extra.</li>
  <li>Beverages and other meals not indicated in the program.</li>
  <li>Early check-in and late check-out at all hotels.</li>
  <li>Travel insurance of all kinds, personal expenses (laundry, telephone, shopping…).</li>
  <li>Gratuities (about 3 USD per person per day).</li>
  <li>Any additional expenses caused by reasons beyond our control such as natural calamities (typhoon, floods), flight delays, rescheduling or cancellations, accidents, medical evacuations, riots, strikes, etc.</li>
  <li>Surcharge for peak season: New Year (01–05 Jan), Lunar New Year (25 Jan–04 Feb), Nation Day (30 Apr–01 May), Independence Day (02 Sep), Christmas (20–31 Dec).</li>
</ul>
...
<li>Child below 100 cm complimentary.</li>
</ul>
